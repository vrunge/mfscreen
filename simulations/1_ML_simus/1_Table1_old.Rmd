...
```{r packages}
knitr::opts_chunk$set(echo = TRUE, eval = TRUE, cache.path = "table1_cache_old_test/")
library(parallel)
```
...
# Old model-free screening test

The following function implements the original marginal screening test directly
from the model-free screening procedure. It accepts the simulated design
matrix `X` and response vector `Y`, computes the marginal slopes, White
variance estimates, studentized statistics, and selected variables.
...
screening_test_ML <- function(X, Y, q_n = 0.10) {
  ...
  if (!is.numeric(q_n) || length(q_n) != 1L || q_n <= 0 || q_n >= 1) {
    stop("q_n must be one number strictly between 0 and 1.")
  }

  x_means <- colMeans(X)
  y_mean <- mean(Y)

  X_centered <- sweep(X, 2L, x_means, FUN = "-")
  Y_centered <- Y - y_mean

  # Marginal slope estimates: tau_hat(j).
  sxx <- colSums(X_centered^2)
  if (any(sxx <= 0)) {
    stop("Every column of X must have strictly positive sample variance.")
  }

  tau_hat <- colSums(X_centered * Y_centered) / sxx
  b_hat <- y_mean - tau_hat * x_means

  # One residual column for each marginal regression:
  # residual[i, j] = Y[i] - b_hat[j] - X[i, j] * tau_hat[j].
  fitted_values <- sweep(X, 2L, tau_hat, FUN = "*")
  fitted_values <- sweep(fitted_values, 2L, b_hat, FUN = "+")
  residuals_hat <- sweep(-fitted_values, 1L, Y, FUN = "+")

  # White estimator:
  # V_hat(j) = { n^{-1} sum X_centered(i,j)^2 residual(i,j)^2 } /
  #            { n^{-1} sum X_centered(i,j)^2 }^2.
  var_x_n <- sxx / n
  V_hat <- colMeans(X_centered^2 * residuals_hat^2) / var_x_n^2
...
This function generates one Table 1 data set, immediately applies the old
model-free screening test, and returns only the compact outputs needed for
the Monte Carlo summaries.
...
one_replication_summary <- data.frame(
  ...
  p = p,
  q = 0.10,
  threshold_gamma = one_result$threshold,
...
The function below runs \(N\) replications for a fixed sample size. It stores
only compact screening outputs, not the large \(X\) and \(Y\) objects.
                              ...
                              base_seed = 20260701L)
{
  ...
  selected_positive <- vapply(
    repetitions, `[[`, numeric(1), "selected_positive"
  )
  selected_null <- vapply(
    repetitions, `[[`, numeric(1), "selected_null"
  )
  selected_total <- vapply(
    repetitions, `[[`, numeric(1), "selected_total"
  )
...
This chunk runs the simulation for \(n=500,1000,2000,2500\), with \(N=500\)
Monte Carlo replications per sample size. For a quick test, temporarily set
`N <- 5L` in the parameter section.
...
table1_summary <- data.frame(
  ...
  Mean_selected_model_size = vapply(
    table1_runs, `[[`, numeric(1), "mean_selected_model_size"
  )
)
...
knitr::kable(
  ...
  align = c("c", "c", "c", "c"),
  caption = paste0(
    "Generated Table 1 using the original model-free screening test ",
    "(N = ", N, ", p = ", p, ", q = 0.10)"
  )
)
```
...
selection_first18_table <- data.frame(
  ...
  do.call(
    cbind,
    lapply(table1_runs, function(x) x$selection_frequency_first18)
  ),
  check.names = FALSE
)
...
saveRDS(
  ...
  file = "table1_old_screening_results.rds"
)

write.csv(
  table1_summary,
  file = "table1_old_screening_summary.csv",
  row.names = FALSE
)
```
