...
# Larger-n competitor simulation for the model-free screening statistic.
#
# Methods:
#   1. mfscreen: reciprocal model-free screening statistic.
#   2. Pearson SIS: marginal Pearson correlation screening.
#   3. Spearman SIS: marginal rank-correlation screening.
#
# Metrics:
#   FPR = false positives / inactive variables.
#   SSP = indicator that all active variables are selected, averaged over
...
out_dir <- file.path(script_dir, "fpr_ssp_n2000_results")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

lib_dir <- file.path(tempdir(), "mfscreen_competitor_sim_n2000")
...
make_response <- function(X, model) {
  if (model == "Parametric linear") {
    beta <- c(0.22, -0.20, 0.18, -0.16, 0.14)
    return(drop(X[, active] %*% beta) + rnorm(nrow(X), sd = 2.3))
  }
