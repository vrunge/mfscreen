...
out_dir <- file.path(script_dir, "mfscreen_vs_mfscreen2_high_np_results")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

lib_base <- file.path(tempdir(), "mfscreen_high_np_base")
lib_fast <- file.path(tempdir(), "mfscreen_high_np_fast")
dir.create(lib_base, recursive = TRUE, showWarnings = FALSE)
dir.create(lib_fast, recursive = TRUE, showWarnings = FALSE)
...
unload_mfscreen <- function() {
  if ("package:mfscreen" %in% search()) {
    detach("package:mfscreen", unload = TRUE, character.only = TRUE)
  }
  if ("mfscreen" %in% loadedNamespaces()) {
    unloadNamespace("mfscreen")
  }
}

time_implementation <- function(lib_dir, X, y, q, n_rep) {
  unload_mfscreen()
  library("mfscreen", lib.loc = lib_dir, character.only = TRUE)

  first_result <- screening_test_matrix(X, y, q = q)
  gc()
  ...
  repeat {
    elapsed <- system.time({
      for (i in seq_len(batch_calls)) {
        invisible(screening_test_matrix(X, y, q = q))
      }
    })[["elapsed"]]
...
for (case in seq_len(nrow(grid))) {
  ...
  base <- time_implementation(lib_base, X, y, q, n_rep)
  fast <- time_implementation(lib_fast, X, y, q, n_rep)
...
if (requireNamespace("ggplot2", quietly = TRUE)) {
  ...
  p_time <- ggplot(
    ...
    labs(
      ...
      title = "High-n mfscreen vs mfscreen2 elapsed time",
  ...
  p_speedup <- ggplot(
    ...
    labs(
      ...
      y = "n",
      fill = "Speedup",
      title = "High-n mfscreen2 speedup over mfscreen"
    ) +
    theme_bw(base_size = 11)
